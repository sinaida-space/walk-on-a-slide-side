...
takeaway. If a slide has two, split it into two slides. A slide that
needs "and" in its headline is two slides.

**Assertion headlines.** The title is a full sentence stating the point,
not a topic label.

- label: "Q3 revenue" → assertion: "Revenue grew in every region in Q3"
- label: "Competitors" → assertion: "No competitor covers both segments"
- label: "Next steps" → assertion: "We need sign-off on the budget by
  Friday"
...
- **Business case / consulting:** recommendation first (executives), or
  problem → structure → evidence → recommendation → ask (working session).
  Match the framework chosen in Stage 1.
- **Talk / pitch:** setup → tension → resolution → proof → ask. Rehearse
  the open and the close; they carry the room.
- Put the hardest objection on its own slide and answer it, rather than
  hoping no one raises it.
...
20-minute talk is ~15 content slides. If the spine is longer, some rows
are really speaker notes — move them off the deck.

## Output

`spine.md`. Show it to the user. Walk the headlines aloud as a paragraph
and ask: does this argue the thing? Fix it here. Every change after
