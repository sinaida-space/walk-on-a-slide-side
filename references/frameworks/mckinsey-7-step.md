...
The seven steps are the thinking order. The **deck order** is not the
same — for an executive audience you lead with the recommendation and
support it backwards. Decide which you need before writing `content.md`.
...
4. **Work plan.** Think efficiency. For each live issue: the analysis
   that would resolve it, the data it needs, who does it, by when.
...
**Executive / decision audience — recommendation first:**
...
| Risks & mitigations | the real objections, answered |
...
Each finding needs a source and an explicit implication or it does not go
on a slide. If a finding has no "so what", drop it.
