...
- the type faces (Geist Pixel for display and body; Libre Franklin only
  where Cyrillic is needed — Geist Pixel is Latin only and fails loudly);
...
the unit. Then still run the grid maths below to derive columns, fields
and the type area for the deck canvas — the house style gives you the
*materials*, Müller-Brockmann gives you the *structure*.
...
- **body size** — 20–24 px reference for a room; 18 only for a deck read
  on screens;
...
`grid.py` does not guess. From canvas + margins + body + leading + fields
it computes, exactly as the book does:
...
theme that binds them — `section` padding = margins, `h1` = headline
size, a background grid overlay you can toggle with a class while
checking alignment).
...
- Turn on the grid overlay. Every text baseline sits on a grid line.
  Images, if any, span whole fields.
- If the test slide overflows, the fix is fewer words (back to Stage 2)
  or a larger field count — not a smaller type size chosen by hand.

## Output

`system/tokens.json`, `system/theme.css`. These are inputs to Stage 4 and
are not edited by hand afterward; change the flags and re-run.
